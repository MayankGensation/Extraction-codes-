...
driver.get("https://sec.bihar.gov.in/ForPublic/ResultP2.aspx")
...
csv_file = "up_mukhya_parshad_data2.csv"
all_data = []
...
# Target post
try:
    select_dropdown_by_text("#ctl00_ContentPlaceHolder1_ddl_Post", "उप मुख्य पार्षद")
except:
    print("❌ Could not select post 'उप मुख्य पार्षद'")
    driver.quit()
    exit()
    ...
    try:
        ...
        for np_type in types:
            ...
            for body in bodies:
                ...
                if headers and rows:
                    for row in rows:
                        all_data.append([district, np_type, body] + row)

                    # Save progress to CSV
                    df = pd.DataFrame(all_data, columns=["District", "Nagar Type", "Nagar Name"] + headers)
