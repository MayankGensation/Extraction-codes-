...
csv_file = "gram_panchayat_sadasya_results(11).csv"
completed_entries = set()
if os.path.exists(csv_file):
    existing_data = pd.read_csv(csv_file, encoding="utf-8-sig")
    for _, row in existing_data.iterrows():
        completed_entries.add(tuple(row[:4]))  # Store completed (District, Block, Panchayat, Ward)

# Select "ग्राम पंचायत के सदस्य"
try:
    select_dropdown_option('//*[@id="ctl00_ContentPlaceHolder1_ddl_Post"]', "ग्राम पंचायत के सदस्य")
except Exception as e:
    print(f"❌ Failed to select 'ग्राम पंचायत के सदस्य': {e}")
    driver.quit()
    exit()
...
if "गोपालगंज" in districts:
    ...
    print("❌ 'गोपालगंज' not found in dropdown options. Exiting.")
    driver.quit()
    exit()
...
for district in districts:
    ...
    for block in blocks:
        ...
        for panchayat in panchayats:
            ...
            wards = get_dropdown_options('//*[@id="ctl00_ContentPlaceHolder1_ddlWard"]')

            for ward in wards:
                if (district, block, panchayat, ward) in completed_entries:
                    print(f"⏭️ Skipping already extracted: {district} > {block} > {panchayat} > {ward}")
                    continue

                select_dropdown_option('//*[@id="ctl00_ContentPlaceHolder1_ddlWard"]', ward)
                ...
                try:
                    ...
                    if table:
                        ...
                        completed_entries.add((district, block, panchayat, ward))  # Mark as completed
                        all_data.clear()  # Clear buffer after saving
                    else:
                        print(f"⚠️ No table found for {district} > {block} > {panchayat} > {ward}")
...
# Close WebDriver
driver.quit()
print("✅ Extraction complete! Data saved to 'gram_panchayat_sadasya_results(11).csv'")
